...
export async function getStaticProps({ params }) {
  const response = await fetch(`http://localhost:3331/administrador/${params.id}`);
...
export async function getStaticPaths() {
  const response = await fetch(`http://localhost:3331/administrador`);
...
export default function Edit({ dados }) {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        await Axios.put(`http://localhost:3331/administrador/${dados.id}`, values);
