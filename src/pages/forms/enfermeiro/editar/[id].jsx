...
import { InputCpf, InputDate, InputPhone, InputText, InputCRM, InputUF } from '../../InputMask';

export async function getStaticProps({ params }) {
  const response = await fetch(`http://localhost:3331/enfermeiro/${params.id}`);
...
export async function getStaticPaths() {
  const response = await fetch(`http://localhost:3331/enfermeiro`);
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      coren: Yup
        ...
        .min(6, 'COREN deve ter no mínimo 6 caracteres'),
      uf: Yup
        .string()
    ...
    onSubmit: async (values) => {
      try {
        await Axios.put(`http://localhost:3331/enfermeiro/${dados.id}`, values);
          ...
          <CardContent sx={{ pt: 0 }}>
              ...
              >
                ...
                >
                  <TextField
                    ...
                    onBlur={formik.handleBlur}
                    onChange={formik.handleChange}
                    value={formik.values.coren}
                    InputProps={{
                      inputComponent: InputCRM,
                    }}
                  />
                  {formik.touched.coren && formik.errors.coren ? (
                    <Typography color="error">{formik.errors.coren}</Typography>
                  ) : null}
                </Grid>
                <Grid
                  xs={12}
                ...
                >
                  <InputUF
                    ...
                    onBlur={formik.handleBlur}
                    onChange={formik.handleChange}
                    value={formik.values.uf.toUpperCase()}
                  />
                  {formik.touched.uf && formik.errors.uf ? (
                    <Typography color="error">{formik.errors.uf}</Typography>
                  ) : null}
                </Grid>
              </Grid>
            </Box>
            ...
            >
              ...
              <Typography
                ...
                variant="body2"
              >
                Insira os dados de acesso do enfermeiro.
              </Typography>
              <Divider sx={{ my: 3 }} />
            </Box>
            <Box sx={{ mt: 3 }}>
